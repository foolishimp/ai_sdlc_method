...
1. `.ai-workspace/{impl}/context/` — tenant-specific (where `{impl}` is the current platform: `claude`, `gemini`, `codex`, `bedrock`)
...
| `interoceptive_signal` | sensory service (Lambda/EventBridge) | Internal monitor detects delta (test regression, coverage drop, build break) |
| `exteroceptive_signal` | sensory service (Lambda/EventBridge) | External monitor detects delta (CVE, dependency update, upstream API change) |
| `affect_triage` | affect pipeline | Signal classified by severity and routed (reflex/escalate/defer) |
| `draft_proposal` | sensory service (Lambda/EventBridge) | Homeostatic response drafted as proposal (requires human approval) |
