...
elif [ -n "${BEDROCK_PLUGIN_ROOT:-}" ] && [ -f "${BEDROCK_PLUGIN_ROOT}/config/graph_topology.yml" ]; then
  TOPOLOGY="${BEDROCK_PLUGIN_ROOT}/config/graph_topology.yml"
fi
...
# Inject context for the agent — stdout goes to the agent
cat <<EOF
