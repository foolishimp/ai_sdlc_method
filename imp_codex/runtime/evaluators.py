...
def _evaluate_agent_check(
    ...
    *,
    ...
    def outcome(result: str, message: str) -> dict:
        return {
            "name": check_name,
            "type": "agent",
            "result": result,
    ...
    if check_name in {"correct_key_format"}:
        return outcome("pass" if req_key else "fail", "feature key matches REQ format" if req_key else "missing REQ key")
    if check_name in {"traces_to_intent"}:
        ok = bool((feature_doc or {}).get("intent")) or "Traces To" in spec_text
        return outcome("pass" if ok else "fail", "intent linkage present" if ok else "missing intent linkage")
    if check_name in {"all_intent_aspects_covered", "requirements_are_testable", "acceptance_criteria_specific"}:
        ...
        return outcome("pass" if ok else "skip", "requirements document references feature with acceptance criteria" if ok else "requirements artifact not available")
    if check_name == "no_ambiguous_language":
        if not spec_text:
            return outcome("skip", "requirements artifact not available")
        return outcome("pass" if not ambiguous_words.search(spec_text) else "fail", "requirements language checked for ambiguity")
    if check_name == "document_has_required_sections":
        ...
        return outcome("pass" if ok else "skip", "requirements sections verified" if ok else "full requirements document not available")
    if check_name == "terminology_defines_domain_terms":
        return outcome("pass" if "Terminology" in spec_text else "skip", "terminology section present" if "Terminology" in spec_text else "terminology section not found")
    if check_name == "success_criteria_measurable":
        return outcome("pass" if "Success Criteria" in spec_text else "skip", "success criteria section present" if "Success Criteria" in spec_text else "success criteria section not found")
    if check_name == "acceptance_criteria_bound_to_feature":
        bound = bool((feature_doc or {}).get("constraints", {}).get("acceptance_criteria"))
        return outcome("pass" if bound else "skip", "feature acceptance criteria bound" if bound else "no bound acceptance criteria")
    if check_name in {"all_reqs_traced_to_components", "no_orphan_components"}:
        ...
        return outcome("pass" if ok else "fail", "design traceability verified" if ok else "design traceability missing")
    if check_name == "interfaces_consistent":
        ok = "Interfaces" in design_text
        return outcome("pass" if ok else "skip", "design interfaces present" if ok else "interfaces section not found")
    if check_name == "dependencies_sound":
        ok = "Dependencies" in design_text
        return outcome("pass" if ok else "skip", "design dependencies present" if ok else "dependencies section not found")
    if check_name == "data_models_present":
        if req_key and req_key.startswith("REQ-DATA-"):
            return outcome("pass" if "Data Model" in design_text else "fail", "data model present" if "Data Model" in design_text else "data model missing")
        return outcome("skip", "not a data requirement")
    if check_name.endswith("_resolved"):
        ...
        return outcome("pass" if resolved else "fail", f"{dimension_name} resolved" if resolved else f"{dimension_name} unresolved")
    if check_name == "advisory_dimensions_considered":
        return outcome("skip", "advisory dimensions not heuristically evaluated")
    if check_name in {"code_matches_design"}:
        ok = bool(design_text) and bool(code_text)
        return outcome("pass" if ok else "fail", "design and code artifacts present" if ok else "missing design or code artifact")
    if check_name in {"follows_coding_standards", "code_quality"}:
        ok = bool(trace["forward"].get("code")) and not tags["untagged_code"]
        return outcome("pass" if ok else "fail", "code present and tagged" if ok else "code missing or untagged")
    if check_name in {"test_quality"}:
        ok = bool(trace["forward"].get("tests")) and not tags["untagged_tests"]
        return outcome("pass" if ok else "fail", "tests present and tagged" if ok else "tests missing or untagged")
    if check_name == "req_tags_present":
        ...
        return outcome("pass" if ok else "fail", "REQ tags detected in generated artifacts" if ok else "REQ tags missing from generated artifacts")
    if check_name in {"all_req_keys_covered", "all_req_keys_have_tests"}:
        ok = bool(trace["forward"].get("tests"))
        return outcome("pass" if ok else "fail", "test coverage detected for feature" if ok else "no tests traced to feature")
    if check_name in {"valid_req_references", "req_keys_exist_in_spec"}:
        ...
        return outcome("pass" if ok else "fail", "tagged REQ keys exist in spec" if ok else "orphan REQ references found")
    if check_name in {"code_files_tagged"}:
        return outcome("pass" if not tags["untagged_code"] else "fail", "all code files tagged" if not tags["untagged_code"] else "untagged code files detected")
    if check_name in {"test_files_tagged"}:
        return outcome("pass" if not tags["untagged_tests"] else "fail", "all test files tagged" if not tags["untagged_tests"] else "untagged test files detected")
    if check_name == "code_req_keys_have_telemetry":
        ok = bool(trace["forward"].get("telemetry"))
        return outcome("pass" if ok else "fail", "telemetry tags detected" if ok else "telemetry tags missing")
    return outcome("skip", "no heuristic implemented for this agent check")
...
def run_agent_checks(paths: RuntimePaths, edge: str, *, feature: str | None = None) -> list[dict]:
    """Run heuristic agent checklist entries for an edge."""

    results = []
    for check in load_edge_checklist(paths, edge):
        if check.get("type") != "agent":
            continue
        results.append(_evaluate_agent_check(paths, edge, check, feature=feature))
