...
2. Removes all other .ai-workspace/ and .gemini/ content (templates, commands, agents)
3. Installs fresh from the specified version (tag)

This addresses stale installations that retain old folders and commands that were
renamed or deleted.
...
class ResetInstaller(InstallerBase):
    ...
    def _resolve_source(self) -> bool:
        ...
        if self.source_path:
            ...
            self.templates_root = self.source_path / "gemini-code" / "project-template"
        ...
        try:
            ...
            self.templates_root = self.temp_dir / "ai_sdlc_method" / "gemini-code" / "project-template"
    ...
    def _validate_target(self) -> bool:
        if not self.target.exists():
            ...
            return False
        has_gemini = (self.target / ".gemini").exists()
        has_workspace = (self.target / ".ai-workspace").exists()
        if not has_gemini and not has_workspace:
            print(f"   Warning: Target has no existing .gemini or .ai-workspace")
    ...
    def _install_fresh(self) -> bool:
        ...
        gemini_source = self.templates_root / ".gemini"
        gemini_target = self.target / ".gemini"
        if gemini_source.exists():
            try:
                ...
                self.print_error(f"Failed to install .gemini/: {e}")
                return False
        else:
            self.print_error(f".gemini/ template not found at {gemini_source}")
            return False
    ...
    def _print_summary(self) -> None:
        ...
        if self.backup_dir:
            print(f"   Backup location: {self.backup_dir}")
        print(f"\n   What was done:")
        print(f"      - Removed old .gemini/ and .ai-workspace/ (framework code)")
        ...
        print(f"      2. Verify installation: ls .gemini/commands/")
