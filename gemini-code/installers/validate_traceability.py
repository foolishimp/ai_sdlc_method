...
Usage:
    ...
    python validate_traceability.py --code installers/ gemini-code/plugins/
...
class TraceabilityValidator:
    ...
    def __init__(self):
        ...
        self.design_refs: Dict[str, List[Tuple[str, int]]] = defaultdict(list)
        self.code_refs: Dict[str, List[Tuple[str, int]]] = defaultdict(list)
        self.test_refs: Dict[str, List[Tuple[str, int]]] = defaultdict(list)

    def extract_requirements(self, requirements_dir: Path) -> None:
        """Extract all requirement keys from the specified directory."""
                ...
                for req_id in matches:
                    ...
                    if req_id not in self.requirements:
                        self.requirements[req_id] = {
                            'file': str(md_file.relative_to(cwd)),
                            'line': line_num,
        ...
        for md_file in design_dir.glob("**/*.md"):
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                for match in self.REQ_PATTERN.finditer(line):
                    req_id = match.group(1)
                    self.all_requirements.add(req_id)
                    if req_id not in [ref[0] for ref in self.design_refs.get(req_id, [])]:
                        self.design_refs[req_id].append((rel_path, line_num))
    ...
    def validate(self) -> Tuple[List[str], List[str], List[str]]:
        ...
        unimplemented = [req_id for req_id in self.requirements if req_id not in self.code_refs]
        untested = [req_id for req_id in self.requirements if req_id not in self.test_refs]
        all_refs = set(self.design_refs.keys()) | set(self.code_refs.keys()) | set(self.test_refs.keys())
        orphaned = [req_id for req_id in all_refs if req_id not in self.requirements]
    ...
    def generate_matrix(self) -> str:
        ...
        for req_id in sorted(self.all_requirements):
            desc = self.requirements[req_id]['description'][:40] if req_id in self.requirements else "(not documented)"
            documented = "✅" if req_id in self.requirements else "❌"
            design = f"✅ ({len(self.design_refs.get(req_id, []))})" if req_id in self.design_refs else "❌"
            code = f"✅ ({len(self.code_refs.get(req_id, []))})" if req_id in self.code_refs else "❌"
            tests = f"✅ ({len(self.test_refs.get(req_id, []))})" if req_id in self.test_refs else "❌"
    ...
    def print_report(self) -> int:
        ...
        if unimplemented:
            ...
            for req_id in sorted(unimplemented)[:10]:
                req = self.requirements[req_id]
        ...
        if untested:
            ...
            for req_id in sorted(untested)[:10]:
                req = self.requirements[req_id]
                print(f"   - {req_id}: {req['description'][:60]}")
            if len(untested) > 10:
...
def main():
    ...
    parser.add_argument('--code', nargs='+', type=Path,
                       default=[Path('installers'), Path('gemini-code/plugins'), Path('.gemini'), Path('templates')],
