# ROOCODE.md - ai_sdlc_method Configuration

Guidance for Roo Code when working in this repository. Follow the AI SDLC methodology and keep requirement traceability intact.

## Context to Load First
- Read `docs/design/roo_aisdlc/README.md` for the Roo Code-specific solution overview.
...
- Roo Code solution design lives in `docs/design/roo_aisdlc/` (implementation design, architecture, ADRs).
- Implementation code lives in `roo-code-iclaude/` (iclaude = implemented by Claude).
...
- Apply the Seven Questions (see "Before You Code" below); stop if any answer is "no."
...
2) Align to stage: identify the current SDLC stage and persona implied by the task; pull details from `docs/requirements/AI_SDLC_REQUIREMENTS.md` and `docs/design/roo_aisdlc/AISDLC_IMPLEMENTATION_DESIGN.md` as needed.
...
- Roo Code solution design: `docs/design/roo_aisdlc/` (README.md, AISDLC_IMPLEMENTATION_DESIGN.md, design.md, adrs/).
- Implementation: `roo-code-iclaude/` (plugins/, installers/, project-template/).
...
### Custom Modes
When working on AISDLC stages, activate the appropriate mode:
- `aisdlc-requirements` - Requirements gathering and REQ key generation
- `aisdlc-design` - Architecture, ADRs, component design
- `aisdlc-tasks` - Task breakdown and work planning
- `aisdlc-code` - TDD implementation (RED-GREEN-REFACTOR)
- `aisdlc-system-test` - BDD integration testing
- `aisdlc-uat` - Business validation
- `aisdlc-runtime` - Telemetry, monitoring, feedback loops

### Memory Bank
Use `.roo/memory-bank/` for persistent context:
- `projectbrief.md` - Project goals and scope
- `techstack.md` - Technology decisions
- `activecontext.md` - Current work focus

### Tool Groups
Each stage mode enables appropriate tools:
- **read**: All stages
- **edit**: Design, Tasks, Code stages
- **command**: Code, System Test stages
- **browser**: Requirements, UAT stages
...
- Divergences from Claude/Codex implementations must be documented in `docs/design/roo_aisdlc/adrs/`.
