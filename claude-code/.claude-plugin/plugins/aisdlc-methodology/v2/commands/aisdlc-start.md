...
| 3 | `NEEDS_INTENT` | No intent file in `docs/specification/INTENT.md` OR intent file is empty/placeholder | → Intent Authoring (Step 3) |
...
Delegate to `/aisdlc-init` with detected values. Write intent description to `docs/specification/INTENT.md`.
...
Write resolved values to `.ai-workspace/context/project_constraints.yml`.
...
Write the response to `docs/specification/INTENT.md` using the standard intent template. Emit `project_initialized` event if not already emitted.
