# The AI SDLC Journey: Setup to UAT (Claude Code)
...
This is your **happy path guide** for **Claude Code** showing:
...
**Who is this for?**
- Claude Code users wanting to see the complete flow
- Teams evaluating the methodology
- Anyone needing a "steel thread" reference

**Time to complete**: 2-3 hours (following along)

> **Note**: For other platforms, see:
> - [Roo Code Guide](../../roo-code-iclaude/guides/JOURNEY.md) - Mode-based AI development
> - [Codex Guide](../../codex-code/guides/JOURNEY.md) - CLI-based workflow
> - [Gemini Guide](../../gemini-code/guides/JOURNEY.md) - Google AI integration
...
- Git installed
- Python 3.8+ (for running installers)
...
There are **three entry points** depending on your situation:

| Entry Point | Description | Time |
|-------------|-------------|------|
| **Path A** | New project from scratch | 15 min |
| **Path B** | Existing project, adding AISDLC | 10 min |
| **Path C** | Existing AISDLC project, updating to latest | 5 min |

Choose your path below.

---

### Step 0: Choose Your Plugin Source (All Paths)

Plugins are deployed via `settings.json` configuration - Claude Code discovers and loads them automatically. Choose one:

**Option A: GitHub Repository (Recommended)**
No cloning needed - point directly to GitHub:
...
**Option B: Local Clone (Development/Offline)**
Clone and point to local directory:
...
> **Note**: See [Plugin Installation](#plugin-installation) in the appendix and [ADR-006](../../docs/design/claude_aisdlc/adrs/ADR-006-plugin-configuration-and-discovery.md) for complete details.

---

### Path A: New Project from Scratch

**Situation**: You're starting a brand new project and want to use AISDLC from day one.

**Step A1: Create Your Project**
...
**Step A2: Configure Plugins via settings.json**

Create `.claude/settings.json` with your plugin configuration (from Step 0):
...
**Step A3: (Optional) Create Workspace for Task Management**

If you want the `.ai-workspace/` task management structure:
...
**Step A4: Verify Installation**

```bash
# Check the structure
ls -la
# Expected: .claude/settings.json (required), .ai-workspace/ (optional)

# Restart Claude Code to load plugins
# Verify plugins loaded via /plugin command
```

**Done!** Your new project is ready with AISDLC. Jump to [The Feature We're Building](#the-feature-were-building).

---

### Path B: Existing Project, Adding AISDLC

**Situation**: You have an existing codebase and want to add AISDLC methodology.

**Step B1: Navigate to Your Project**

```bash
cd /path/to/your-existing-project
```

**Step B2: Configure Plugins**

**Option 1: Use the settings installer (Recommended)**

```bash
# This creates/updates .claude/settings.json with AISDLC plugins
python ~/ai_sdlc_method/claude-code/installers/setup_settings.py --target . --source github
```
...
**Step B3: (Optional) Create Workspace for Task Management**
...
💡 Your existing code is untouched. AISDLC workspace is added alongside your project.

**Done!** Your existing project now has AISDLC. Jump to [The Feature We're Building](#the-feature-were-building).

---

### Path C: Existing AISDLC Project, Updating to Latest

**Situation**: Your project already has AISDLC installed, and you want to update to the latest version.

Since plugins are loaded from GitHub (or your configured source), updating is automatic:

**For GitHub source**: Plugins update automatically when you restart Claude Code.

**For local directory source**: Update your local clone:
...
**Done!** No installer needed - Claude Code loads plugins directly from the source.

---

### Understanding the Implicit Session Model

**Important**: Claude Code uses an **implicit session model** - context loads automatically when you open Claude Code. There is no need to run a session start command.

**What Auto-Loads**:
1. **CLAUDE.md** - Project guide and methodology overview
2. **ACTIVE_TASKS.md** - Your current tasks and work context
...
| Command | Description |
|---------|-------------|
| `/aisdlc-status` | Show current session status |
| `/aisdlc-checkpoint-tasks` | Save your work (updates task status, creates finished docs) |
| `/aisdlc-update` | Update AISDLC to latest version |
| `/aisdlc-refresh-context` | Reload context if needed |
| `/aisdlc-todo "desc"` | Add a new task |
| `/aisdlc-finish-task <id>` | Mark task as complete |

---

### Verify Your Setup

Regardless of which path you took, verify everything is working:

```bash
# 1. Check structure
ls -la .claude/ .ai-workspace/
...
# 4. Test traceability tool (from ai_sdlc_method repo)
...
│ Tester: Product Owner                                       │
│ Result: ✅ APPROVED                                         │
│ Sign-off: Jane Smith, 2025-11-26                           │
│                                                              │
...
2. **Deploy to Production (Stage 7)**
   - See [AI SDLC Method](../../docs/ai_sdlc_method.md) Section 10: Runtime Feedback Stage
   - Set up telemetry with REQ-* tagging
   - Configure alerts that trace to requirements
   - Close the feedback loop

3. **Customize the Methodology**
   - See [Plugin Guide](../../docs/guides/PLUGIN_GUIDE.md)
...
- 🏗️ [New Project Setup](../../docs/guides/NEW_PROJECT_SETUP.md) - Detailed setup for your project (30 min)

**Complete Methodology**:
- [AI SDLC Method](../../docs/ai_sdlc_method.md) - Complete 7-stage specifications (3,300+ lines)
...
## Appendix: Claude Code Commands Reference
...
Plugins are installed via Claude Code's marketplace system. See the official documentation:
- **Plugins**: https://code.claude.com/docs/en/plugins
- **Plugin Reference**: https://code.claude.com/docs/en/plugins-reference
- **Settings Reference**: https://code.claude.com/docs/en/settings

**Method 1: Local Development (Directory Source)**

Run the installer to copy plugins to your project:
```bash
python /path/to/ai_sdlc_method/claude-code/installers/setup_plugins.py --bundle enterprise --force
```
...
**Method 2: GitHub Repository Source**

For teams using the GitHub repository directly:
```json
{
  "extraKnownMarketplaces": {
    "aisdlc": {
...
**Method 3: Interactive Plugin Management**
...
After editing `settings.json`, restart Claude Code to load plugins.

**Step 5: Verify Installation**

Run `/plugin` to verify plugins are installed:
...
- The marketplace has `.claude-plugin/marketplace.json`
- Each plugin has `.claude-plugin/plugin.json` with valid schema
- Plugin `source` paths start with `./`
...
| `aisdlc-methodology` | Complete 7-stage SDLC agent configurations |
...
### AISDLC Workflow Commands

```bash
# Task management (workspace installed)
/aisdlc-status                  # Show current session status
/aisdlc-todo "description"      # Add a new task
/aisdlc-finish-task <id>        # Mark task as complete
/aisdlc-checkpoint-tasks        # Save all work (creates finished docs)

# Update and maintenance
/aisdlc-update                  # Update AISDLC to latest version
/aisdlc-refresh-context         # Reload context if needed
/aisdlc-release                 # Bump version and create release

# Note: /aisdlc-start-session has been removed - sessions start implicitly
```
...
```bash
# Configure settings.json with AISDLC plugins
python claude-code/installers/setup_settings.py --target /path/to/project --source github
python claude-code/installers/setup_settings.py --target /path/to/project --source github --all     # all plugins
python claude-code/installers/setup_settings.py --target /path/to/project --source github --bundle enterprise
python claude-code/installers/setup_settings.py --target /path/to/project --source directory       # local dev
python claude-code/installers/setup_settings.py --target /path/to/project --source github --dry-run  # preview
