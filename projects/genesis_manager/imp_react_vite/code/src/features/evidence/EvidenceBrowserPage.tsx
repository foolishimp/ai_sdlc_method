    ...
    <div className="h-screen flex flex-col bg-gray-50 overflow-hidden">
      ...
      <header className="flex-shrink-0 bg-white border-b px-4 h-14 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="font-semibold text-gray-900">Evidence Browser</span>
        </div>
        <div className="flex items-center gap-3">
          <FreshnessIndicator
      ...
      <div className="flex-1 overflow-hidden grid grid-cols-2 divide-x divide-gray-200">
        ...
        <div className="overflow-y-auto p-4 flex flex-col gap-6">
          <section>
            <h2 className="text-sm font-semibold text-gray-700 mb-3">Traceability Coverage</h2>
            <TraceabilityTable
              workspaceId={id}
        ...
        <div className="overflow-hidden flex flex-col divide-y divide-gray-200">
          {/* Event list */}
          <div className="flex-1 overflow-y-auto p-4">
            <h2 className="text-sm font-semibold text-gray-700 mb-3">
              Event History
              {selectedFeatureId && <span className="font-mono text-blue-700 ml-2">{selectedFeatureId}</span>}
            </h2>
            {eventsLoading ? (
                  ...
                  {events.map((evt) => (
                    <tr
                      ...
                      className={`cursor-pointer hover:bg-gray-50 ${
                        selectedEventIndex === evt.eventIndex ? 'bg-blue-50' : ''
                      }`}
                    >
                      <td className="py-1.5 pr-2 text-xs text-gray-400 font-mono">{evt.eventIndex}</td>
                      <td className="py-1.5 pr-2 text-xs font-medium text-gray-700">{evt.eventType}</td>
                      <td className="py-1.5 pr-2 text-xs text-gray-500">{evt.edge ?? '—'}</td>
