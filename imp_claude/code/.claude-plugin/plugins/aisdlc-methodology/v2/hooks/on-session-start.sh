...
# Reference: /aisdlc-start Step 10, ADR-017 (Functor Execution Model)
# ============================================================================
...
if [ -f "$STATUS_FILE" ] && [ -f "$EVENTS_FILE" ]; then
  ...
  if [ "$STATUS_MTIME" -lt "$EVENTS_MTIME" ]; then
    ISSUES="${ISSUES}  [i] STATUS.md is older than events.jsonl — consider running /aisdlc-status\n"
