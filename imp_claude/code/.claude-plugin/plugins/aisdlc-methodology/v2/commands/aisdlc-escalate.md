# /aisdlc-escalate - Escalation Queue Management
...
```
/aisdlc-escalate [--feature "REQ-F-*"] [--severity critical|high|medium|all] [--action]
```
...
HIGH:
  [H1] INTENT  INT-OBS-042 "Missing test coverage for REQ-NFR-SEC-003"
       Signal source: gap (from /aisdlc-gaps)
  ...
  [H2] REVIEW  REQ-F-API-001 on requirements→design
       ...
       Recommended: run /aisdlc-spec-review --feature "REQ-F-API-001"
...
Actions:
  1. Process item     — /aisdlc-escalate --action
  2. Dismiss item     — provide reason (logged to events)
  3. Spawn vector     — /aisdlc-spawn from escalation
  4. Review feature   — /aisdlc-spec-review or /aisdlc-review
...
Choose action:
  ...
  4. Escalate to human review — /aisdlc-spec-review
...
## Integration with /aisdlc-start

The `/aisdlc-start` state machine checks for escalations as part of state detection:
