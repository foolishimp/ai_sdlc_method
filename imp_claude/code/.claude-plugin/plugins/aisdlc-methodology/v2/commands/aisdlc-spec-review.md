# /aisdlc-spec-review - Formal Spec-Boundary Review

Structured review protocol for spec-boundary edges where the asset crosses from one specification level to another (intent→requirements, requirements→design). Extends `/aisdlc-review` with gradient checking — does the transformation preserve intent while binding to the target level's constraints?
...
```
/aisdlc-spec-review --feature "REQ-F-*" --edge "{source}→{target}" [--diff] [--checklist-only]
```
...
For non-spec-boundary edges (code↔unit_tests, uat_tests), use `/aisdlc-review` instead.
...
## Relationship to /aisdlc-review

`/aisdlc-review` is the general-purpose human gate — it works on any edge.
`/aisdlc-spec-review` is the **specialised** review for spec-boundary edges — it adds the gradient check (completeness, fidelity, boundary) that is meaningless on non-spec edges.

The iterate agent should delegate to `/aisdlc-spec-review` when:
...
For all other edges, delegate to `/aisdlc-review`.
