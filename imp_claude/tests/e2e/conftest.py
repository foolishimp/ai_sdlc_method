...
def _write_status_md(archive_dir: pathlib.Path) -> None:
    """Generate .ai-workspace/STATUS.md from feature vectors for genesis_monitor.

    genesis_monitor's project list reads the Phase Summary pipe-table in STATUS.md
    for the Phase/Convergence columns. gen-iterate is supposed to write it, but
    skips it in headless mode. This generates a deterministic equivalent so every
    archived e2e run is visible in the genesis_monitor dashboard.
    """
    try:
        import yaml  # pyyaml is a test dependency
    except ImportError:
        return

    ws = archive_dir / ".ai-workspace"
    features_active = ws / "features" / "active"
    if not features_active.exists():
        return

    # Map trajectory asset-node keys → display edge names
    ASSET_TO_EDGE: dict[str, str] = {
    ...
    rows: list[tuple[str, str, int]] = []  # (edge_name, status, iterations)
    for yml_path in sorted(features_active.glob("*.yml")):
        try:
        ...
        if not isinstance(traj, dict):
            continue
        for asset_key, edge_data in traj.items():
            if not isinstance(edge_data, dict):
                continue
            edge_name = ASSET_TO_EDGE.get(asset_key, asset_key)
            raw_status = str(edge_data.get("status", "not_started"))
            status = STATUS_MAP.get(raw_status, "not_started")
            iters = edge_data.get("iterations") or edge_data.get("iteration", 0)
            try:
                iters = int(iters)
            except (TypeError, ValueError):
                iters = 0
            rows.append((edge_name, status, iters))

    if not rows:
        return

    now = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    done = sum(1 for _, s, _ in rows if s == "converged")
    total = len(rows)

    table_lines = [
        "| Edge | Status | Iterations |",
        "|------|--------|-----------|",
    ] + [f"| {e} | {s} | {i} |" for e, s, i in rows]

    content = "\n".join([
        "# Project Status",
        f"*Generated by e2e test framework: {now}*",
        "",
        "## Phase Summary",
        "",
        *table_lines,
        ...
        "## Summary",
        "",
        f"- Edges converged: {done}/{total}",
        f"- Generated: {now}",
        "",
    ])
    try:
        (ws / "STATUS.md").write_text(content)
    except OSError:
        pass
...
def _persist_run(source_dir: pathlib.Path, failed: bool = False) -> pathlib.Path | None:
    ...
    try:
        ...
        # Generate STATUS.md from feature vectors for genesis_monitor compatibility
        _write_status_md(dest)
        # Write manifest
