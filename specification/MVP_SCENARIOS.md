...
| GAP-004 | **genesis_monitor schema drift + single-project discovery.** Two sub-gaps: **(a) Schema drift** — the monitor (v2.5) recognises 9 event types. Current methodology (v2.8/3.0) emits 12+ types. Field structures have drifted for `edge_converged` (expects `convergence_time`, gets `iteration` + `convergence_type`) and `iteration_completed` (expects flat evaluator map, gets nested object). 4 new event types (`edge_started`, `exteroceptive_signal`, `gaps_validated`, `release_created`) render as raw untyped JSON. **(b) Single-project discovery** — `--watch-dir` expects one `.ai-workspace/` root. Cannot scan a directory of projects (e.g. `imp_claude/tests/e2e/runs/` where each subdirectory is a full project with its own `.ai-workspace/events/events.jsonl`). Multi-project scanning (glob for `*/.ai-workspace/events/events.jsonl`) is needed for the e2e run archive and for monitoring multiple real projects simultaneously. | Dashboard displays incomplete/degraded information. Cannot observe historical e2e runs or multiple projects from a single invocation. | genesis_monitor upgrade. (a) ~2-3h: add 4 event types, update 3 field extractors. (b) ~1-2h: recursive project discovery via `.ai-workspace/` marker directories. |
...
| GAP-004 | genesis_monitor schema drift + single-project discovery | SC-001 | Medium | genesis_monitor upgrade (schema + multi-project scanning) |
