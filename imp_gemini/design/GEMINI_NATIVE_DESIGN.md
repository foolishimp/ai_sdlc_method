# AI SDLC — Restated Design (v2.8)

This document restates the AI SDLC methodology as a pure instantiation of the Asset Graph Model, free of legacy constraints or historical comparisons.
...
## 6. Multi-Tenant Implementations

The methodology is platform-agnostic. We maintain two primary tenants:

### imp_gemini (Local)
*   **State**: Local filesystem (`.ai-workspace/events/events.jsonl`).
*   **Compute**: Local CLI / Python scripts.
*   **Intelligence**: Gemini via API.
*   **Coordination**: Single-user, sequential.

### imp_gemini_cloud (GCP-Native)
*   **State**: **Firestore** (materialized view) + **BigQuery** (immutable log).
*   **Messaging**: **Pub/Sub** (the methodology's nervous system).
*   **Compute**: **Cloud Run** (stateless orchestrators).
*   **Intelligence**: **Vertex AI (Gemini 1.5 Pro)**.
*   **Coordination**: Multi-agent concurrent via Firestore atomic transactions.
