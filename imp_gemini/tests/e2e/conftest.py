...
class MockGeminiRunner:
    ...
    def run_auto(self, feature_id):
        ...
        for edge in ["intent→requirements", "requirements→design", "design→code", "code↔unit_tests"]:
            run_id = str(uuid.uuid4())
            self._emit_event("edge_started", feature=feature_id, edge=edge, runId=run_id)
            if "code" in edge or "unit_tests" in edge: self._generate_artifacts(feature_id)
            self._emit_event("iteration_completed", feature=feature_id, edge=edge, iteration=1, delta=0, status="converged", runId=run_id)
