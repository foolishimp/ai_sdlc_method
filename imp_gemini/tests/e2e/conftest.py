...
def _write_status_md(archive_dir: pathlib.Path) -> None:
    """Generate .ai-workspace/STATUS.md for genesis_monitor compatibility."""
    ...
    for yml_path in sorted(features_active.glob("*.yml")):
        try:
            ...
            for asset_key, edge_data in traj.items():
                ...
                status = edge_data.get("status", "not_started")
    ...
    table_lines = ["| Edge | Status | Iterations |", "|------|--------|-----------|"]
    table_lines += [f"| {e} | {s} | {i} |" for e, s, i in rows]
    
    content = "\n".join([
        "# Project Status",
        f"*Generated by e2e archival: {now}*",
        "",
        "## Phase Summary",
        "",
        *table_lines
...
def _persist_run(source_dir: pathlib.Path, failed: bool = False) -> pathlib.Path | None:
    """Copy project dir to runs/ archive with metadata and symlinks."""
    try:
        ...
        print(f"E2E: Archived run ({'FAILED' if failed else 'OK'}) \u2192 {dest}", flush=True)
        return dest
