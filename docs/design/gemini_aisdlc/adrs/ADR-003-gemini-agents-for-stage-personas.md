...
We need a mechanism to give Gemini stage-appropriate context and perspective.

---

## Decision

**We will use Gemini Code's agent system (`.claude/agents/*.md`) to provide stage-specific personas for each SDLC stage.**
...
Gemini (generic): "Okay, here's some code for authentication..."
...
```
You: "Create requirements for authentication"
Gemini (Requirements Agent persona): "I'll extract structured requirements with unique keys...
...
Gemini: [Reads aisdlc-requirements-agent.md]
...
Gemini: [Needs full instructions every time]
```
...
**Approach**: One Gemini assistant for all stages
...
**Approach**: Run 7 different Gemini instances, one per stage
...
```
AGENTS = WHO Gemini is (persona/role)
...
Flow:
1. Gemini reads: aisdlc-requirements-agent.md (AGENT = WHO)
...
- Gemini thinks like appropriate role (BA, Dev, QA, etc.)
...
- Gemini doesn't auto-switch stages
...
# Gemini reads aisdlc-code-agent.md
