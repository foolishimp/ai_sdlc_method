...
**Depends On**: ADR-001 (Gemini Code as Platform)

---

## Context

Given Gemini Code as the MVP platform (ADR-001), we need a mechanism to integrate common development workflows (starting sessions, managing tasks, committing work) directly into the AI conversation.
...
**We will use Gemini Code's slash command system (`.claude/commands/*.md`) to integrate workflows directly into AI conversation.**
...
```
Without commands:
Developer: Works in Gemini
Developer: Switches to terminal → git commit
Developer: Switches to Jira → updates ticket
Developer: Switches back to Gemini
Developer: Re-explains context to Gemini
= 5 context switches

With commands:
Developer: Works in Gemini
Developer: /aisdlc-commit-task 5
= 0 context switches (Gemini handles git + task updates)
```

**2. AI-Native Workflow**
- Commands execute within AI conversation
- Gemini maintains context throughout
...
- Gemini Code convention: `.claude/commands/*.md`
...
**Decision**: Use Gemini Code's built-in command system rather than invent our own
...
```bash
# Separate tool outside Gemini
$ aisdlc task create "Implement auth"
$ aisdlc task complete 5
```

**Why Rejected**:
- ❌ Context-switching (leave Gemini, run CLI, return)
- ❌ Gemini doesn't know about CLI actions
- ❌ Separate installation/maintenance
- ❌ Breaks conversational flow

### Alternative 2: Natural Language Only
```
"Gemini, please update task 5 to completed and create finished documentation"
```

**Why Rejected**:
- ❌ Inconsistent (different users phrase differently)
- ❌ Verbose (requires full sentence vs /command)
- ❌ Error-prone (Gemini might misinterpret)
...
- ❌ Not MVP scope
- ❌ Context-switching (browser vs Gemini)
...
- **ADR-001**: Gemini Code as Platform (foundation for commands)
