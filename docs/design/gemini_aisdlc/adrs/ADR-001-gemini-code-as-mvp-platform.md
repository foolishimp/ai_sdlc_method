# ADR-001: Gemini Code as MVP Implementation Platform
...
1. **Gemini Code native** - Use Gemini Code's plugin system, commands, and agents
...
**We will use Gemini Code's native plugin system as the MVP implementation platform.**

Specifically:
- **Plugins** - Deliver methodology via `.claude-plugin/` format
- **Commands** - Workflow integration via `.gemini/commands/` slash commands
- **Agents** - Stage personas via `.gemini/agents/` markdown files
...
### Why Gemini Code (vs Alternatives)

**1. Native Platform Leverage** ✅
- Gemini Code already has plugin system - we use it, not build it
...
- Edit markdown → reload Gemini → test
...
- ❌ Extension marketplace different from Gemini plugins
...
- ❌ Context-switching between CLI and Gemini
...
✅ **Native Integration**
- Commands work seamlessly in Gemini Code
- Agents load automatically
- Skills available via plugins
- No glue code needed

✅ **Federated Architecture**
- Leverage Gemini Code's marketplace system
...
⚠️ **Gemini Code Dependency**
- Locked to Gemini Code for MVP
- Other LLMs require future work (MCP server)
- Gemini Code changes could break plugins
...
- Must follow Gemini Code plugin conventions
- Limited to capabilities Gemini Code provides
- Can't extend platform itself

**Mitigation**:
- Constraints are reasonable
- Gemini Code platform is extensible
- Feedback to Google for improvements
...
1. **Gemini Code Platform** (t = 2025-11-25)
   - Plugin system: Stable, documented
   - Command system: `.gemini/commands/*.md` format
   - Agent system: `.gemini/agents/*.md` format
   - Marketplace: GitHub-based, local file support

2. **Google Roadmap**
   - Gemini Code under active development
...
3. **Competitive Landscape**
   ...
   - Gemini Code - Most extensible for methodology delivery
...
|-------------|------------|-----|
| REQ-F-PLUGIN-001 | ✅ | Gemini Code has native plugin system |
...
| REQ-F-CMD-001 | ✅ | Slash commands via `.gemini/commands/` |
| REQ-F-CMD-002 | ✅ | Agents via `.gemini/agents/` |
