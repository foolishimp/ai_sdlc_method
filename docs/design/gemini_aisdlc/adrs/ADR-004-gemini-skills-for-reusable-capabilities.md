...
**Depends On**: ADR-001 (Gemini Code as Platform), ADR-003 (Agents for Personas)

---

## Context

Agents provide stage-specific **personas** (WHO Gemini is), but agents need **capabilities** (HOW to accomplish tasks). Many capabilities are reusable across multiple stages and agents.
...
- **Cross-cutting** = Skills available to any agent (or Gemini directly)
...
┌──────────────────────────────────────────────────────────┐
│                    AGENT LAYER                            │
│  (WHO Gemini is + Stage Responsibilities)                │
├──────────────────────────────────────────────────────────┤
...
**Why Rejected**:
- ❌ Context-switching (leave Gemini, run script, return)
- ❌ Gemini can't invoke programmatically
...
**Approach**: Just give Gemini prompts like "extract requirements"
...
- ❌ Language-specific
- ❌ Adds complexity
- ❌ Gemini Code doesn't execute arbitrary code
...
- Gemini interprets and executes
...
- **ADR-001**: Gemini Code as Platform (skills delivered via plugins)
...
1. AGENT: aisdlc-code-agent.md loads
   → Gemini thinks like a Developer
